...
const messageTpl = `
	...
	{{ else }}
		if all {
			switch v := interface{}({{ accessor . }}).(type) {
				case interface{ ValidateAll() error }:
					if err := v.ValidateAll(); err != nil {
						errors = append(errors, {{ errCause . "message.generic" "err" "embedded message failed validation" }})
					}
			...
			if err := v.Validate(); err != nil {
				return {{ errCause . "message.generic" "err" "embedded message failed validation" }}
			}
		}
	{{ end }}
`
